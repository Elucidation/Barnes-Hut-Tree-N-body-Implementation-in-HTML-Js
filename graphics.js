...
function drawBNnode(node, depth) {
  ...
  if (typeof node.b != "undefined" && depth <= BN_DRAW_DEPTH) {
    // Draw Node
    drawBBOX(node.box[0], node.box[1], node.box[2], node.box[3]);
    c.textBaseline = "top";
  ...
  if (INTERACTION_METHOD == "BN") {
    data.innerHTML +=
      ...
      "<br/>\n\
			...
			<p>\n\
			Speedup : " +
      (100 * (1 - numChecks / bruteHalfChecks)).toFixed(2) +
      "%<br/>\n\
			</p>";
  }
  ...
  if (DEBUG >= 1) {
    ...
    for (i = 0; i < bods.N; i++) {
      data.innerHTML +=
        ...
        i +
        ": Pos " +
        bods.pos.x[i].toFixed(2) +
        ", " +
        bods.pos.y[i].toFixed(2) +
        "; M: " +
        bods.mass[i] +
        " </li>";
    }
...
function massToRadius(mass) {
  return (
    MINRADIUS +
    ((mass - MINMASS) / (MAXMASS - MINMASS)) * (MAXRADIUS - MINRADIUS)
  );
}
...
function drawArrow(x, y, x2, y2, h, color) {
  ...
  c.moveTo(x2, y2);
  c.lineTo(
    x2 - h * Math.cos(angle - Math.PI / 8),
    y2 - h * Math.sin(angle - Math.PI / 8)
  );
  c.lineTo(
    x2 - h * Math.cos(angle + Math.PI / 8),
    y2 - h * Math.sin(angle + Math.PI / 8)
